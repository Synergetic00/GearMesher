...
def getRotatedSVG(pathSVG, degrees=0):
    ...
    originalSVG = originalSVG.rotate(degrees)
...
def loadSVGShit(pathSVG, rot=0):
    if rot != 0:
        getRotatedSVG(pathSVG, rot)
        doc = minidom.parse('temp.svg')  # parseString also exists
    else:
...
subjArr = loadSVGShit('gear2.svg', 50)
cirlceArrr = loadSVGShit('circle2.svg')
temp = getPointArrayTempo(subjArr)
tempOff = getPointArrayTempo(cirlceArrr, 300, 300)
# print(temp)
...
result = c.Execute(ClipType.Difference, solution, pft, pft)
